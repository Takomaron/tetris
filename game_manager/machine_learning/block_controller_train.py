        ...
        for i in range(1, self.tetris_fill_height):
            ...
            if reshape_board[self.height - i][0] == 0:  # 左端が空いている
                if self.get_line_right_fill(reshape_board, sum_, i):    # 左端以外は埋まっている
                    ...
                    """ ## Try29 ４行消しできる状態かどうかチェックするため、下からの連続左空け数だけカウントする。
...
###        reward = self.reward_list[lines_cleared] * (1 + (self.height - max(0,max_height))/self.height_line_reward)
        ...
        ## 穴がないか、一番低い穴が５より高い位置にあるのに３行以下を削除したら、ペナルティ
        ## ４行消しすると、左空け報酬が激減する。激減するので４行消しになるのを待たずに３行消しするのかも
        ## ４行消しより、左空け状態を継続した方が報酬が高いとすれば、４行消ししないだろう。
        ## Max高さの高いときに行消しすると報酬を高くする。それだと、一部だけ高くなるかも。
        ## 左空け報酬の大きいときに、行消し報酬も大きくできればいいのではないだろうか。
        ## 左空け報酬の小さいときに、行消ししても報酬は小さく。
        ## それで、４行消しする前と後で左空け報酬に差がないようにしたい。
        ## 左空け報酬はいつも同じにする？すると、一段目だけは左空けキープするようになるかも
        ## 左空け高さの高さに比例して行消しさせると、高い位置で消すようになってしまうかもしれない。
        ## ４行消しを優先して、ゲームオーバーにならないように進めてほしい
        ## ４行消しは、どこの高さで消してもらってもいい
...
###                reward += max(tetris_reward * self.tetris_fill_reward, self.reward_list[3] * (1 + (self.height)/self.height_line_reward))
        ...
        reward += tetris_reward * self.tetris_fill_reward
        if (curr_shape_class.shape == 1 or hold_shape_class.shape == 1) and tetris_reward >= 4 and lines_cleared != 4:
            ### Try31.7 4行消ししないペナルティをパラメータ化
            reward -= tetris_reward * self.reward_weight[5]
            """
            reward -= tetris_reward * self.tetris_fill_reward
            """
        ...
        reward += self.tetris_fill_reward * self.cleared_col[4] * self.reward_weight[4]
